...
@Component
public class ForgotPasswordAuditPolicy
    ...
    public Object onReturned(Object[] arguments, Object result) {
        ...
        return switch(commandResult) {
            ...
            case Result.Success<?> ignored ->
                new ForgotPasswordAuditEvent.Requested(
                        normalizedEmail
                );
            case Result.Failure<?> failure ->
                new ForgotPasswordAuditEvent.Rejected(
                        normalizedEmail,
                        reasonFor(failure.error().code())
                );
        };
    }
    ...
    public Object onThrown(Object[] arguments, Throwable throwable) {
        return new ForgotPasswordAuditEvent.Failed(
                normalizedEmail(arguments),
                "system_error"
        );
    }
    ...
    private String normalizedEmail(Object[] arguments) {
        if (arguments.length == 0 ||
                !(arguments[0] instanceof String email) ||
                email.isBlank()) {
            return "invalid";
        }
