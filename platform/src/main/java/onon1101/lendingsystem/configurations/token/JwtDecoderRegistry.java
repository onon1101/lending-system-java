...
final class JwtDecoderRegistry implements JwtDecoderProvider {
    ...
    JwtDecoderRegistry(JwtDecoderFactory factory, List<TokenProperties> tokenProperties) {
        Map<String, JwtDecoder> result = new HashMap<>();

        for (TokenProperties properties : tokenProperties) {
            JwtDecoder previous =
                    result.putIfAbsent(properties.purpose(), factory.create(properties));

            if (previous != null) {
                throw new IllegalStateException(
                        "Duplicate JWT token purpose: " + properties.purpose());
            }
        }
    ...
    public JwtDecoder getDecoder(String purpose) {
        ...
        if (decoder == null) {
            throw new IllegalArgumentException("No JWT decoder registered for purpose: " + purpose);
        }
